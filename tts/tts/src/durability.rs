...
use crate::guest::{TtsVoicesGuest, TtsSynthesisGuest, TtsStreamingGuest, TtsAdvancedGuest};

pub struct DurableTts<Impl> {
    phantom: PhantomData<Impl>,
}

pub trait ExtendedGuest: TtsVoicesGuest + TtsSynthesisGuest + TtsStreamingGuest + TtsAdvancedGuest + 'static {}

#[cfg(not(feature = "durability"))]
mod passthrough_impl {
    use super::*;
    use crate::exports::golem::tts::types::TtsError as WitTtsError;
    use crate::exports::golem::tts::voices::{LanguageInfo as WitLanguageInfo, VoiceFilter as WitVoiceFilter, VoiceInfo as WitVoiceInfo};
    use crate::exports::golem::tts::synthesis::{SynthesisOptions as WitSynthesisOptions, ValidationResult as WitValidationResult};
    use crate::exports::golem::tts::streaming::{StreamSession as WitStreamSession, StreamStatus as WitStreamStatus};
    use crate::exports::golem::tts::advanced::{AudioSample as WitAudioSample, VoiceDesignParams as WitVoiceDesignParams, PronunciationEntry as WitPronunciationEntry, LongFormJob as WitLongFormJob, LongFormResult as WitLongFormResult};
    use crate::golem::tts::types::{TextInput as WitTextInput, TimingInfo as WitTimingInfo, SynthesisResult as WitSynthesisResult, AudioChunk as WitAudioChunk};
    ...
    impl<Impl: ExtendedGuest> WitSynthesisGuest for DurableTts<Impl> {
        ...
        fn get_timing_marks(input: WitTextInput, voice_id: String) -> Result<Vec<WitTimingInfo>, WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
            Impl::get_timing_marks(input, voice_id)
        }

        fn validate_input(input: WitTextInput, voice_id: String) -> Result<WitValidationResult, WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
    ...
    impl<Impl: ExtendedGuest> WitAdvancedGuest for DurableTts<Impl> {
        ...
        fn design_voice(name: String, characteristics: WitVoiceDesignParams) -> Result<String, WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
        ...
        fn create_lexicon(name: String, language: String, entries: Option<Vec<WitPronunciationEntry>>) -> Result<String, WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
            Impl::create_lexicon(name, language, entries)
        }

        fn add_lexicon_entry(lexicon_id: String, entry: WitPronunciationEntry) -> Result<(), WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
        ...
        fn synthesize_long_form(content: String, voice_id: String, output_location: String, chapter_breaks: Option<Vec<u32>>) -> Result<WitLongFormJob, WitTtsError> {
            LOGGING_STATE.with_borrow_mut(|state| state.init());
