...
pub fn test_silence_handling() -> String {
    ...
    match transcription::transcribe(&silent_audio, config, None) {
        ...
        Err(err) => match err {
            types::SttError::InvalidAudio(_) => "OK silence: rejected as invalid".to_string(),
...
pub fn test_long_audio_handling() -> String {
    ...
    match transcription::transcribe(&long_audio, config, None) {
        ...
        Err(err) => match err {
            types::SttError::QuotaExceeded(_) => "OK long: quota limit hit".to_string(),
            types::SttError::UnsupportedOperation(_) => "OK long: length limit".to_string(),
...
pub fn test_network_error_handling() -> String {
    ...
    match transcription::transcribe(&audio, config, None) {
        ...
        Err(err) => match err {
            types::SttError::NetworkError(_) => "OK network: error detected".to_string(),
            types::SttError::ServiceUnavailable(_) => "OK network: service unavailable".to_string(),
...
pub fn test_rate_limiting() -> String {
    ...
    for _i in 0..5 {
        match transcription::transcribe(&audio, config, None) {
            Ok(_) => continue,
            Err(types::SttError::RateLimited(_)) => {
                rate_limited = true;
                break;
            }
            Err(_) => continue,
...
pub fn test_quota_behavior() -> String {
    ...
    match transcription::transcribe(&large_audio, config, None) {
        ...
        Err(err) => match err {
            types::SttError::QuotaExceeded(info) => {
                format!("OK quota: exceeded {}/{} {}", info.used, info.limit, 
                       match info.unit {
                           types::QuotaUnit::Seconds => "seconds",
                           types::QuotaUnit::Requests => "requests", 
                           types::QuotaUnit::Credits => "credits",
                       })
            }
            types::SttError::InsufficientCredits => "OK quota: insufficient credits".to_string(),
...
pub fn test_invalid_audio_formats() -> String {
    ...
    match transcription::transcribe(&invalid_audio, config, None) {
        ...
        Err(err) => match err {
            types::SttError::InvalidAudio(_) => "OK invalid: rejected".to_string(),
            types::SttError::UnsupportedFormat(_) => "OK invalid: format rejected".to_string(),
...
pub fn test_streaming_after_finish() -> String {
    ...
    let mut stream = match transcription::transcribe_stream(config, None) {
        Ok(s) => s,
